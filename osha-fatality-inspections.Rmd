...
current_date <- str_remove_all(Sys.Date(), "-") # Set current date

data_path <- "data"
data_dir <- "osha_inspections"
data_url <- "https://enfxfr.dol.gov/data_catalog/OSHA"
data_file <- paste0("osha_inspection_", current_date, ".csv.zip")
...
osha_inspections_w_workplace <- osha_inspections %>%
  filter(safety_hlth == safety) %>%
  mutate(estab_address = str_replace_all(paste0(
...
n_distinct(osha_inspected_prior %>%
  select(estab_address, open_date_all))
```
...
year_before_covid_end_date <- "2019-07-18"
